...
void VKGL_APIENTRY OpenGL::vkglUniformMatrix2fv(GLint          location,
                                                GLsizei        count,
                                                GLboolean      transpose,
                                                const GLfloat* value)
{
    ...
    VKGL_TRACE("glUniformMatrix2fv(location=[%d] count=[%d] transpose=[%d] value=[%p])",
               location,
               static_cast<int32_t>(count),
               (transpose == GL_TRUE) ? 1 : 0,
               value);

    dispatch_table_ptr->pGLUniformMatrix2fv(dispatch_table_ptr->bound_context_ptr,
                                            location,
                                            count,
                                            transpose,
                                            value);
                                                  ...
                                                  const GLfloat*   in_value_ptr)
{
    if (validate(in_context_ptr,
                 ...
                 in_value_ptr) )
    {
        vkglUniformMatrix2fv_execute(in_context_ptr,
                                     in_location,
                                     in_count,
                                     in_transpose,
                                     in_value_ptr);
